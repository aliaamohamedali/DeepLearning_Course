...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = g*e.dot(g)/(np.square(np.linalg.norm(g)))
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = bias_axis*mu.dot(bias_axis)/(np.square(np.linalg.norm(bias_axis)))
    mu_orth = mu-mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = bias_axis*e_w1.dot(bias_axis)/(np.square(np.linalg.norm(bias_axis)))
    e_w2B = bias_axis*e_w2.dot(bias_axis)/(np.square(np.linalg.norm(bias_axis)))
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(1-np.square(mu_orth))*((e_w1B-mu_B)/(e_w1-mu_orth-mu_B))
    corrected_e_w2B = np.sqrt(1-np.square(mu_orth))*((e_w2B-mu_B)/(e_w2-mu_orth-mu_B))
